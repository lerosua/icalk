...
class Buddy {
	public:
		...
		void set_jid(const JID& jid_) { jid = jid_; }
		/**返回JID的bare（）值*/
		const std::string& get_jid() const { return jid.bare(); }
		/** 返回JID*/
		 ...
		 * @brief 获取当前的签名
		 */
		const std::string& get_sign_msg() { return sign_msg; }
		/**
		 * @brief 设置签名信息
		 * @param msg_ 需要设置的签名信息
		 */
		void set_sign_msg(std::string msg_) { sign_msg =msg_; }

		/**
		 * @brief 获取所属的组
		 */
		StringList getGroups() { return groups; }
		 ...
		 * @return 返回此Buddy的会话
		 */
		MessageSession* get_session() { return session; }
		...
		void	setLogo(Glib::RefPtr<Gdk::Pixbuf >logo_) { logo = logo_ ; }
